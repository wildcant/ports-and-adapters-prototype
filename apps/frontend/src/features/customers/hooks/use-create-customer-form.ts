import { CreateCustomer, type CustomerListResponse } from '@proteus/http-schemas'
import { useCreateCustomer } from '#/features/customers/api/customers'
import type { SubmitFormParams } from '#/lib/form.ts'
import { useAppForm } from '#/lib/form-hook.ts'

export type CreateCustomerFormParams = SubmitFormParams<CustomerListResponse>
...
export function useCreateCustomerForm(params?: CreateCustomerFormParams) {
  ...
  const form = useAppForm({
    ...
    validators: { onSubmit: CreateCustomer },
    onSubmit: ({ value }) => {
