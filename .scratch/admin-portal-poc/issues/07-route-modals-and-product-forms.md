...
- Medusa source — route-based modal components (port structure, adapt to shadcn Drawer):
...
- Proteus — existing form pattern to follow:
  - `apps/frontend/src/components/form/text-field.tsx` — TextField with `useFieldContext`, shadcn Input, FieldError
- shadcn Drawer (both slide-in and full-screen variants):
  - https://ui.shadcn.com/docs/components/base/drawer

- [ ] `RouteDrawer` component wraps shadcn Drawer with `swipeDirection="right"`, provides `onClose` that navigates to parent route, compound children (`.Header`, `.Title`, `.Body`, `.Footer`)
- [ ] `RouteFocusModal` component wraps shadcn Drawer in full-screen variant, provides `onClose` that navigates to parent route, compound children (`.Header`, `.Title`, `.Body`, `.Footer`)
...
- [ ] `TextField` component using `useFieldContext<string>()`, shadcn Input, FieldLabel, FieldError — matching the frontend pattern
