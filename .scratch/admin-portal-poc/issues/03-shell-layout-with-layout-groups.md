...
- Medusa source — shell and navigation (port the structure and styling, adapt to shadcn):
...
- [ ] Shell component uses shadcn Sidebar: collapsible, "Products" nav link using TanStack Router's `<Link>`, app name/logo area
- [ ] Sidebar highlights the active nav item based on current route
- [ ] Topbar renders above the content area
- [ ] Breadcrumb bar uses shadcn Breadcrumb component, reads from `useMatches()` collecting `staticData.breadcrumb` and `context.breadcrumb`
